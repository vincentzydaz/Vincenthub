...
if not isfolder("Vincent Hub") then
    makefolder("Vincent Hub")
end
...
function Library:clear()
	for _, object in CoreGui:GetChildren() do
		if object.Name ~= "Vincent Hub" then
			continue
		end
		object:Destroy()
	end
...
function Library:save_flags()
    ...
    local success, result = pcall(function()
        ...
        writefile(`Vincent Hub/{game.GameId}.lua`, flags)
    end)
end
...
function Library:load_flags()
    if not SaveFlagsG then return end
    local success, result = pcall(function()
        if not isfile(`Vincent Hub/{game.GameId}.lua`) then
            Library.save_flags()
            return
        end

        local flags = readfile(`Vincent Hub/{game.GameId}.lua`)
        if not flags then
            Library.save_flags()
            return
        end
